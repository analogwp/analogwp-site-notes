...
const TaskDetail = ({ 
    ...
    const handleDelete = async () => {
        if (confirm(__('Are you sure you want to delete this comment?', 'client-handoff-toolkit'))) {
            try {
        ...
        <div className="cht-task-detail">
            <div className="cht-task-detail-header">
                <button 
                    ...
                    title={__('Back to list', 'client-handoff-toolkit')}
                >
                    <svg width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                        <path fillRule="evenodd" d="M15 8a.5.5 0 0 0-.5-.5H2.707l3.147-3.146a.5.5 0 1 0-.708-.708l-4 4a.5.5 0 0 0 0 .708l4 4a.5.5 0 0 0 .708-.708L2.707 8.5H14.5A.5.5 0 0 0 15 8z"/>
                    </svg>
                    {__('Back', 'client-handoff-toolkit')}
                </button>
                ...
                <div className="cht-task-detail-actions">
                    ...
                    >
                        <option value="low">{__('Low Priority', 'client-handoff-toolkit')}</option>
                        <option value="medium">{__('Medium Priority', 'client-handoff-toolkit')}</option>
                        <option value="high">{__('High Priority', 'client-handoff-toolkit')}</option>
                    </select>
                    ...
                    >
                        ...
                        {__('Delete', 'client-handoff-toolkit')}
                    </button>
                </div>
            </div>
            ...
            <div className="cht-task-detail-content">
                <div className="cht-task-detail-main">
                    <div className="cht-task-detail-info">
                        <div className="cht-task-meta">
                            <div className="cht-task-priority">
                                ...
                                <span className="cht-priority-label">
                                    {priority ? priority.charAt(0).toUpperCase() + priority.slice(1) : __('Normal', 'client-handoff-toolkit')}
                                </span>
                            </div>
                        ...
                        <div className="cht-task-text">
                            <h2>{__('Comment', 'client-handoff-toolkit')}</h2>
                            <p>{comment.comment_text}</p>
                        </div>
                        
                        {comment.page_url && (
                            <div className="cht-task-page">
                                <h3>{__('Page URL', 'client-handoff-toolkit')}</h3>
                    ...
                    {comment.screenshot_url && (
                        <div className="cht-task-screenshot-main">
                            <h3>{__('Screenshot', 'client-handoff-toolkit')}</h3>
                            <div className="cht-screenshot-container-main">
                                <img 
                                    src={comment.screenshot_url} 
                                    alt={__('Task screenshot', 'client-handoff-toolkit')}
                    ...
                    {comment.replies && comment.replies.length > 0 && (
                        <div className="cht-task-replies">
                            <h3>{__('Replies', 'client-handoff-toolkit')} ({comment.replies.length})</h3>
                                        ...
                                        <div className="cht-reply-header">
                                            <div className="cht-reply-author">
                                                ...
                                                <span className="cht-reply-name">
                                                    {reply.display_name || __('Unknown User', 'client-handoff-toolkit')}
                                                </span>
                                            </div>
                ...
                <div className="cht-task-detail-sidebar">
                    <div className="cht-task-detail-meta">
                        <h3>{__('Details', 'client-handoff-toolkit')}</h3>
                        
                        <div className="cht-task-user-detail">
                            <label>{__('Created by:', 'client-handoff-toolkit')}</label>
                            <div className="cht-user-info">
                                ...
                                <span className="cht-user-name">
                                    {user?.name || __('Unknown User', 'client-handoff-toolkit')}
                                </span>
                            </div>
                        </div>
                        
                        <div className="cht-task-date-detail">
                            <label>{__('Created:', 'client-handoff-toolkit')}</label>
                            <span>{formatDate(comment.created_at)}</span>
                        </div>
                        
                        <div className="cht-task-id">
                            <label>{__('Comment ID:', 'client-handoff-toolkit')}</label>
